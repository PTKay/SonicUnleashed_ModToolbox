@echo off
cd /d "%~dp0"
title Sonic Unleashed PS3 Mod Toolbox 1.2.1
set is_empty=true
set is_edat=false
for %%f in (#files\*.*) do (
set is_empty=false
...
echo SELECT OPTION
echo ---------------------
echo 1 - Encrypt to EDAT
echo 2 - Decrypt from EDAT
...
if %option% EQU 3 (goto extractar)
...
echo Extracting AR file for decompression...
artools\quickbms.exe -Q artools\arcsys.bms "%%d" #output\DecompressedFiles\out >nul
set filename=%%~nxd
set noar=!filename:.ar.00=!
